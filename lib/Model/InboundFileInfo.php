...
class InboundFileInfo implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setName($name)
    {
        ...
        if ((mb_strlen($name) > 128)) {
            throw new \InvalidArgumentException('invalid length for $name when calling InboundFileInfo., must be smaller than or equal to 128.');
        }
        if ((mb_strlen($name) < 1)) {
            throw new \InvalidArgumentException('invalid length for $name when calling InboundFileInfo., must be bigger than or equal to 1.');
        }
        if ((!preg_match("/^[A-Za-z0-9-_\\.\\[\\]\\{\\}\\(\\)\/ ]+$/", ObjectSerializer::toString($name)))) {
            throw new \InvalidArgumentException("invalid value for \$name when calling InboundFileInfo., must conform to the pattern /^[A-Za-z0-9-_\\.\\[\\]\\{\\}\\(\\)\/ ]+$/.");
        }
